<x-admin.dashboard-layout title="Edit Galeri - Dwi AC Mobil">
    <div class="space-y-6 sm:space-y-8 md:space-y-10 lg:space-y-12">
        ...
        <div class="flex items-center space-x-2 text-sm text-gray-600">
            ...
            <span class="text-gray-900 font-medium">Edit Galeri</span>
        </div>
        ...
        <div class="bg-gradient-to-br from-[#0F044C] via-[#1D2C90] to-[#192BC2] text-white rounded-2xl px-5 py-6 sm:px-7 sm:py-7 shadow-2xl relative overflow-hidden">
            ...
            <div class="relative">
                <h1 class="text-2xl sm:text-3xl font-montserrat-48 font-bold leading-tight uppercase">Edit Galeri</h1>
                <p class="mt-1 text-sm sm:text-base text-white/85 defparagraf">Perbarui foto galeri AC mobil.</p>
            </div>
        </div>
            ...
            <form action="{{ route('admin.galeri.update', $galeri->id_galeri) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text"
                               name="nama_foto"
                               value="{{ old('nama_foto', $galeri->nama_foto) }}"
                    ...
                    <div>
                        ...
                        <div class="flex items-start space-x-4">
                            ...
                            <div class="w-32 h-32 border-2 border-gray-300 rounded-lg flex items-center justify-center bg-gray-50 overflow-hidden">
                                @if($galeri->foto)
                                    <img id="imagePreview" src="{{ asset('storage/galeri/' . $galeri->foto) }}" alt="Current Image" class="w-full h-full object-cover">
                                    <div id="imagePlaceholder" class="text-center hidden">
                    ...
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        <div class="flex items-start space-x-3">
                            ...
                            <div>
                                <h4 class="text-sm font-medium text-blue-800">Informasi Galeri</h4>
                                <div class="text-sm text-blue-700 mt-1">
                                    <p><strong>Tanggal Upload:</strong> {{ $galeri->created_at->format('d M Y, H:i') }}</p>
                                    @if($galeri->updated_at->ne($galeri->created_at))
                                        <p><strong>Terakhir Diperbarui:</strong> {{ $galeri->updated_at->format('d M Y, H:i') }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
