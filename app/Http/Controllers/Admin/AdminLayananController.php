...
class AdminLayananController extends Controller
{
    ...
    public function storeLayanan(Request $request)
    {
        $data = $request->validate([
            ...
            // 'is_active' column is optional in schema; only fill if exists
        ]);
        ...
        // Guard for optional is_active column
        if ($request->has('is_active') && \Schema::hasColumn('layanan', 'is_active')) {
            $payload['is_active'] = (bool)$request->boolean('is_active');
        }

        Layanan::create($payload);

        return redirect()->route('admin.layanan')->with('success', 'Layanan berhasil dibuat');
    }
    ...
    public function storeKategori(Request $request)
    {
        ...
        return redirect()->route('admin.layanan')->with('success', 'Kategori berhasil dibuat');
    }
    ...
    public function destroyKategori($id)
    {
        ...
        if ($layananCount > 0) {
            return redirect()->route('admin.layanan')->with('warning', 
